...
class DNANet_wo_dense_wo_FPFM(nn.Module):
    def __init__(self, num_classes, input_channels, block, num_blocks, nb_filter,deep_supervision=True):
        super(DNANet_wo_dense_wo_FPFM, self).__init__()
        if block    =='Res_block':
            block   = Res_block
            self.block = 'Res_block'
        elif block  =='Res_CBAM_block':
        ...
        self.conv1_2 = self._make_layer(block, nb_filter[1] + nb_filter[2]+ nb_filter[0], nb_filter[1], num_blocks[0])
        self.conv2_2 = self._make_layer(block, nb_filter[2] + nb_filter[3]+ nb_filter[1], nb_filter[2], num_blocks[1])

        self.conv0_3 = self._make_layer(block, nb_filter[0] + nb_filter[1], nb_filter[0])
        self.conv1_3 = self._make_layer(block, nb_filter[1] + nb_filter[2]+ nb_filter[0], nb_filter[1], num_blocks[0])
    ...
    def forward(self, input):
        ...
        x4_0 = self.conv4_0(self.pool(x3_0))
        x3_1 = self.conv3_1(torch.cat([x3_0, self.up(x4_0),self.down(x2_1)], 1))
        x2_2 = self.conv2_2(torch.cat([x2_1, self.up(x3_1),self.down(x1_2)], 1))
        x1_3 = self.conv1_3(torch.cat([x1_2, self.up(x2_2),self.down(x0_3)], 1))
...
if __name__ == '__main__':
    ...
    model = DNANet_wo_dense_wo_FPFM(num_classes=1, input_channels=in_channels, block=Res_block, num_blocks=num_blocks, nb_filter=nb_filter)
